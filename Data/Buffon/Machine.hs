{-|
 ...
 Copyright   : (c) Maciej Bendkowski, 2019-2021
...
init g = case random g of
  (x, g') -> Rand { buffer = x, counter = 0, oracle = g' }
{-# INLINE init #-}
...
samples'
  :: RandomGen g => BuffonMachine g a -> Int -> [a] -> BuffonMachine g [a]
...
histogram
  :: RandomGen g => Discrete g -> Int -> BuffonMachine g [(Int, S.Occur)]
...
oracle' rng | empty rng = init (oracle rng)
            | otherwise = rng
{-# INLINE oracle' #-}
...
rational a b = do
  ...
  heads <- flip
  if v >= b
    then if heads then rational (v - b) b else return True
    else if heads then rational v b else return False
...
choiceDDG :: RandomGen g => Vector Int -> Discrete g
choiceDDG enc = choiceDDG' enc 0

choiceDDG' :: RandomGen g => Vector Int -> Int -> Discrete g
choiceDDG' enc c = do
  h <- flip
  let b  = fromEnum h
  let c' = enc ! (c + b)
  if enc ! c' < 0 then return $ -(1 + enc ! c') else choiceDDG' enc c'
...
binExpansion x = binExpansion' bs n where (bs, n) = floatToDigits 2 x

real' :: RandomGen g => Bin -> Bern g
real' []       = error "Absurd case"
real' (b : bs) = do
  heads <- flip'
  if heads then real' bs else return b
...
-- | Mean combinator.
mean
  :: RandomGen g => BuffonMachine g a -> BuffonMachine g a -> BuffonMachine g a

mean = cond flip

-- | Even-parity combinator.
even :: RandomGen g => Bern g -> Bern g
even p = cond (neg p) (return True) (cond (neg p) (return False) (even p))
...
geometric' p n = cond (neg p) (return n) (geometric' p $ succ n)
...
leaves (Leaf n    ) = ([n] ++)
leaves (Node lt rt) = case (lt, rt) of
...
insertR
  :: RandomGen g => Maybe RandTrie -> Int -> BuffonMachine g (Maybe RandTrie)
...
trie :: RandomGen g => Int -> BuffonMachine g (Maybe RandTrie)
...
vonNeumann test p = do
  ...
  case t of
    ...
    Just t' -> if test (orderType t') then return n else vonNeumann test p

-- | Sorted permutations.
sorted :: [Int] -> Bool
sorted []           = True
sorted [_         ] = True
sorted (n : m : ns) = n <= m && sorted (m : ns)
...
uniform' n !v !c = do
  ...
  if n <= v'
    then if c' < n then return c' else uniform' n (v' - n) (c' - n)
    else uniform' n v' c'
...
toll :: Num a => (Int -> a -> a -> a) -> Int -> DecisionTree b -> a
...
decisionTree ps = decisionTree' (zip nats ps') where ps' = layout ps
...
splits' xs [p] = (p, xs, [])
splits' xs (p : ps) | isCut p   = (p, xs, ps)
...
choice' :: RandomGen g => Bool -> DecisionTree a -> BuffonMachine g a
